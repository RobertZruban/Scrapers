...
url = "https://www.macrotrends.net/stocks/charts/" + "BABA" +"/alibaba/cash-flow-statement"
...
for td in soup.findAll('td'):
    count += 1
    if count % 4 == 0:
        revenue.append(td.get_text())
    elif count % 3 == 0:
        category.append(td.get_text())
    elif count % 2 == 0:
        full_name.append(td.get_text())
    elif count % 1 == 0:
        ticker_list.append(td.get_text())
    if count == 4:
        count = 0
...
for name in full_name:
    ...
    name_replace.append(name.replace(" ", "-"))    
    

for ticker,name in zip(ticker_list,name_replace):
...
for x in soup.findAll('div', {'role' : 'columnheader'}):
    ...
    print(soup.findAll('div', {'role' : 'columnheader'})[j].get_text())
    j +=1
...
soup.findAll('div', {'role' : 'columnheader'})[2].get_text()
